...
# Insert JS before window.addEventListener('scroll'...
insert_pos = content.rfind('        window.addEventListener(\'scroll\', () => {')

if insert_pos != -1:
    new_content = content[:insert_pos] + js_to_add + "\n" + content[insert_pos:]
    with open('index.html', 'w') as f:
        f.write(new_content)
    print("JS animations added.")
else:
